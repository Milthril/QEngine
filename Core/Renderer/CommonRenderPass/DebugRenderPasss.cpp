...
DebugRenderPasss::DebugRenderPasss() {

}

void DebugRenderPasss::setupWindow(QRhiWindow* window) {
	mWindow = window;
	mWindowFrameSize = mWindow->size() * mWindow->devicePixelRatio();
}

void DebugRenderPasss::compile() {
	mRT.colorAttachment.reset(RHI->newTexture(QRhiTexture::RGBA32F, mWindowFrameSize, 1, QRhiTexture::RenderTarget | QRhiTexture::UsedAsTransferSource));
	...
	mDebugPainter.setupWindow(mWindow);
	mDebugPainter.setupRenderTarget(mRT.renderTarget.get());
	mDebugPainter.compile();
}

void DebugRenderPasss::execute() {
	QRhiCommandBuffer* cmdBuffer;
	if (RHI->beginOffscreenFrame(&cmdBuffer) == QRhi::FrameOpSuccess) {
		mDebugPainter.paint(cmdBuffer);
