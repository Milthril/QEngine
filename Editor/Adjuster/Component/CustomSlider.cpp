...
const int scLeftRightMargin = 0;
}

CustomSlider::CustomSlider(QWidget* aParent)
	: QWidget(aParent),
	mMinimum(0),
	mMaximum(100),
	mLowerValue(0),
...
CustomSlider::CustomSlider(Qt::Orientation ori, Options t, QWidget* aParent)
	: QWidget(aParent),
	mMinimum(0),
	mMaximum(100),
	mLowerValue(0),
	...
	mSecondHandlePressed(false),
	mInterval(mMaximum - mMinimum),
	mBackgroudColorEnabled(QColor(100, 100, 100)),
	mBackgroudColorDisabled(Qt::darkGray),
	mBackgroudColor(mBackgroudColorEnabled),
	orientation(ori),
	type(t)
...
QRectF CustomSlider::handleRect(int aValue) const
{
	if (orientation == Qt::Horizontal)
...
void CustomSlider::mousePressEvent(QMouseEvent* aEvent)
{
...
void CustomSlider::mouseMoveEvent(QMouseEvent* aEvent)
{
	if (aEvent->buttons() & Qt::LeftButton)
	{
...
void CustomSlider::mouseReleaseEvent(QMouseEvent* aEvent)
{
	Q_UNUSED(aEvent);
...
void CustomSlider::changeEvent(QEvent* aEvent)
{
...
int CustomSlider::GetMinimun() const
{
	return mMinimum;
}

void CustomSlider::SetMinimum(int aMinimum)
{
	setMinimum(aMinimum);
}

int CustomSlider::GetMaximun() const
{
	return mMaximum;
}

void CustomSlider::SetMaximum(int aMaximum)
{
	setMaximum(aMaximum);
}

int CustomSlider::GetLowerValue() const
{
	return mLowerValue;
}

void CustomSlider::SetLowerValue(int aLowerValue)
{
	setLowerValue(aLowerValue);
}

int CustomSlider::GetUpperValue() const
{
	return mUpperValue;
}

void CustomSlider::SetUpperValue(int aUpperValue)
{
...
void CustomSlider::setLowerValue(int aLowerValue)
{
	...
	else if (aLowerValue < mMinimum) {
		aLowerValue = mMinimum;
	}
	else if (aLowerValue > mUpperValue) {
...
void CustomSlider::setUpperValue(int aUpperValue)
{
	...
	else if (aUpperValue < mMinimum) {
		aUpperValue = mMinimum;
	}
	else if (aUpperValue < mLowerValue) {
...
int CustomSlider::validLength() const
{
...
void CustomSlider::SetRange(int aMinimum, int mMaximum)
{
	setMinimum(aMinimum);
	setMaximum(mMaximum);
}